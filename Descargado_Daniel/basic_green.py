...
def cover_page(doc, year1, month1, volume, no):
    ...
    doc.append(NoEscape(r"\AddToShipoutPicture*{\put(0,0){\includegraphics[scale=0.25]{"+ruta+"bulletfiles/portada1.jpg}}}"))
    #doc.append(NoEscape(r"\centering"))
    doc.append(NoEscape(r"\vspace*{11cm}"))
    doc.append(NoEscape(r"{\huge \textcolor{white}{\hspace*{1cm}\sffamily\selectfont\hspace*{-1.5cm} %s DE %s}}\par"%(month1.upper(), year1)))
    doc.append(NoEscape(r"\par\normalfont\fontsize{35}{35}\sffamily\selectfont"))
    #doc.append(NoEscape(r"{\large \textcolor{white}{\hspace*{-2.7cm}Volumen %s, No. %s}}\par"%(volume, no)))
...
def tittle_page(doc, year1, year2, month1, month2):
    ...
    doc.append(NoEscape(r"\Huge{\textbf{Boletín de sismos: %s %s\\}}"%(month1.capitalize(), year1)))
...
def presentation(doc):
    ...
    doc.append(Command("input",ruta+"bulletfiles/presentacion2.tex"))
...
def functioning(doc, sub_list = [["1"],["1"],["1"]]):
    ...
    # =======================================================================
    if sub_list[1][0] == "1":
		doc.append(NoEscape(r"\section{Mapa de funcionamiento de las estaciones}"))
...
#		doc.append(NoEscape(r"\vspace{-1.0cm}"))
    ...
    if sub_list[2][0] == "1":
		...
		doc.append(NoEscape(r"\begin{center}"))
		doc.append(NoEscape(r"\includegraphics[scale=0.85]{%s}"%sub_list[2][1]))
		doc.append(NoEscape(r"\end{center}"))
...
#		doc.append(NoEscape(r"\vspace{-1.5cm}"))
		doc.append(NoEscape(r"\begin{center}"))
		doc.append(NoEscape(r"\includegraphics[scale=0.18]{%s}\\"%sub_list[1][1]))
...
#		doc.append(NoEscape(r"\vspace{-1.5cm}"))
    ...
    if sub_list[3][0] == "1":
		...
		doc.append(NoEscape(r"\begin{center}"))
#		doc.append(NoEscape(r"\vspace{-1.5cm}"))
		doc.append(NoEscape(r"\includegraphics[scale=0.18]{%s}"%sub_list[3][1]))
...
def graphics(doc, year, month1, sub_list = [["1"],["1"],["1"],["1"]]):
    ...
    if sub_list[0][0] == "1":
		...
		doc.append(NoEscape(r"\begin{center}"))
		doc.append(NoEscape(r"\includegraphics[scale=1.0]{loc_err.png}"))
    ...
    if sub_list[1][0] == "1": 
		...
		file_inspector('mag_sismos.jpg')
		file_inspector('prof_sismos.jpg')
		file_inspector('prof_mag_sismos.jpg')
		image_mag_sismos = os.path.join(os.path.dirname(__file__), 'mag_sismos.jpg')
		image_prof_sismos = os.path.join(os.path.dirname(__file__), 'prof_sismos.jpg')
		image_prof_mag_sismos = os.path.join(os.path.dirname(__file__), 'prof_mag_sismos.jpg')
		...
		doc.append(NoEscape(r"\includegraphics[scale=0.3]{mag_sismos.jpg}"))
		doc.append(NoEscape(r"\includegraphics[scale=0.3]{prof_sismos.jpg}"))
		doc.append(NoEscape(r"\end{multicols}"))
		doc.append(NoEscape(r"\vspace{-0.5cm}"))
		doc.append(NoEscape(r"\begin{center}"))
		doc.append(NoEscape(r"\includegraphics[scale=0.3]{rms_sismos.jpg}"))
    ...
    if sub_list[2][0] == "1":
		...
		doc.append(NoEscape(r"\begin{multicols}{2}"))
		doc.append(NoEscape(r"\includegraphics[scale=0.5]{sismos_vs_tiempo_meses.jpg}"))
		...
		doc.append(NoEscape(r"\caption{No de eventos por meses, días y acumulado por días}"))
		doc.append(NoEscape(r"\end{center}"))
		doc.append(NoEscape(r"\vspace{0.5cm}"))
...
def curiosity_section(doc, sub_list = [["1"],["0"]]):
	...
	doc.append(NoEscape(r"En diciembre de 1993 ya se encontraban\
	 instaladas 13 de las catorce estaciones previstas, donadas por el Gobierno \
	 Canadiense a través de la Agencia Canadiense de Desarrollo \
	 Internacional - ACDI - y del Programa de las Naciones Unidas para \
	 el Desarrollo - PNUD -, TELECOM e INGEOMINAS. El 22 de abril  \
	 se inaugura oficialmente la RSNC con 15 estaciones en funcionamiento."))
	...
	for j in range(1,4):
		...
		if image_name != "":
			...
			doc.append(NoEscape(r"\begin{center}"))
			doc.append(NoEscape(r"\includegraphics[scale=0.5]{%s}"%image_name))
			doc.append(NoEscape(r"\end{center}"))
