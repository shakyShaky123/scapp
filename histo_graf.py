...
plt.subplot(321)
plt.grid('on')
plt.scatter(ml, err_lat, s=30, c='b', alpha=0.5)
...
plt.subplot(322)
plt.grid('on')
plt.scatter(ml, err_lon, s=30, c='b', alpha=0.5)
...
plt.subplot(323)
plt.grid('on')
plt.scatter(ml, err_dep, s=30, c='b', alpha=0.5)
...
plt.subplot(324)
plt.grid('on')
plt.scatter(err_lon, err_lat, s=30, c='r', alpha=0.5)
#plt.plot(err_lon, err_lat, 'bo')
plt.title("Error en latitud vs Error en longitud",fontsize=16)
plt.xlabel('Error en longitud',fontsize=16)
plt.ylabel('Error en latitud',fontsize=16)
plt.ylim(0,max(err_lat)+1)
plt.xlim(0,max(err_lon)+0.1)

plt.subplot(325)
plt.grid('on')
plt.scatter(ml, rms, s=30, c='b', alpha=0.5)
#plt.plot(ml, rms, 'bo')
...
plt.subplot(326)
plt.grid('on')
plt.scatter(dep, err_dep, s=30, c='g', alpha=0.5)
#plt.plot(dep, err_dep, 'bo')
plt.title("Error en profundidad vs profundidad",fontsize=16)
plt.xlabel('Profundidad Z [km]',fontsize=16)
plt.ylabel('Error en profundidad [km]')
plt.ylim(-0.03,max(err_dep)+0.1)
plt.xlim(0,max(dep)+0.1)



output.append('ErrorDepthVSMl.pdf')
#fig.savefig('figura1.pdf',format='pdf')
#fig.suptitle("Todos los eventos registrados en el area  por la RSNC (1993-2013)", fontsize=20)
fig.suptitle("Eventos registrados por el proyecto \n de tectonica en el VMM", fontsize=18)
...
plt.subplot(222)
hist_ml=plt.hist(ml,bins=np.arange(0,5,0.5))
plt.xlabel('$M_l$',fontsize=8)
plt.ylabel('Numero de sismos',fontsize=8)

plt.subplot(223)
hist_nsta=plt.hist(nsta,bins=np.arange(0,12,1))
plt.xlabel('Numero de estaciones',fontsize=8)
plt.ylabel('Numero de sismos',fontsize=8)
output.append('StationsVSEvents.pdf')
plt.suptitle("Eventos registrados por el proyecto \n de tectonica en el VMM", fontsize=16)
#plt.suptitle("Todos los eventos registrados en el area  por la RSNC (1993-2013)", fontsize=20)
plt.savefig(output[1])
#plt.savefig('figura2.pdf')

plt.figure(3)
plt.subplot(221)
hist_errlat=plt.hist(err_lat,bins=np.arange(0,10,1))
plt.xlabel('Error en latitud (km)',fontsize=8)
plt.ylabel('Numero de sismos',fontsize=8)

plt.subplot(222)
hist_errlon=plt.hist(err_lon,bins=np.arange(0,10,1))
plt.xlabel('Error en longitud (km)',fontsize=8)
plt.ylabel('Numero de sismos',fontsize=8)

plt.subplot(223)
hist_errdep=plt.hist(err_dep,bins=np.arange(0,10,1))
...
plt.figure(4)
plt.subplot(111)
#hist_errdep=plt.hist(dep,bins=np.arange(0,130,5))
hist_ml=plt.hist(ml,bins=np.arange(0,5,0.25))
plt.xlabel('$M_l$',fontsize=8)
plt.ylabel('Numero de sismos',fontsize=8)
...
output.append('MlVSEvents_VMM.pdf')
plt.savefig(output[3])

plt.figure(5)
plt.subplot(111)
hist_errdep=plt.hist(rms,bins=6)
